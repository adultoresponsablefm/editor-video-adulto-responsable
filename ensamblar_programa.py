...
def validar_av(video):
    r=subprocess.run(["ffprobe","-v","error","-show_entries","stream=codec_type","-of","csv=p=0",str(video)],capture_output=True,text=True,check=True)
    tipos={x.strip() for x in r.stdout.splitlines()}
    if "video" not in tipos or "audio" not in tipos:
        raise RuntimeError(f"Exportación inválida: se requieren video y audio; pistas encontradas: {sorted(tipos)}")
def main():
    ...
    if not cortes: raise RuntimeError("No hay cortes; no se generará un resultado sin analizar")
    ...
    try:
        ...
        run(["ffmpeg","-y","-hide_banner","-loglevel","error","-f","concat","-safe","0","-i",str(lista),"-map","0:v:0","-map","0:a:0","-c","copy","-movflags","+faststart",str(SALIDA)])
        validar_av(SALIDA)
